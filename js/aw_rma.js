...
var AWRMAAdminRmaFormControl = Class.create({
    initialize: function(globalObject, vf) {
        ...
        /* Save and Update Stock Id */
        this.saveAndUpdateStockId = 'awrma-save-and-update-stock';
        /* Print */
        ...
        this.allowedStatuses = new Array();
    },

    disableSubmitButtons: function() {
        for(var i = 0; i < this.submitButtons.length; i++){
            this.disableButton(this.submitButtons[i]);
        }
        this.disableButton(this.saveAndUpdateStockId);
    },

    enableSubmitButtons: function() {
        for(var i = 0; i < this.submitButtons.length; i++){
            this.enableButton(this.submitButtons[i]);
        }
        this.checkAvailabilityUpdateStockButton();
    },

    enableButton: function(buttonId) {
        if ($(buttonId))
            $(buttonId).removeClassName('disabled').writeAttribute('disabled', null);
    },

    disableButton: function(buttonId) {
        if ($(buttonId))
            $(buttonId).addClassName('disabled').writeAttribute('disabled', 'disabled');
    },
    ...
    observeStatus: function() {
        me = this;
        $(this.statusId).observe('change', this.checkAvailabilityUpdateStockButton);
    },

    setAllowedStatuses: function(allowedStatuses) {
        this.allowedStatuses = allowedStatuses;
    },

    checkAvailabilityUpdateStockButton: function() {
        var currStatus = parseInt($(me.statusId).value);
        if (me.allowedStatuses.indexOf(currStatus) != -1) {
            me.enableButton(me.saveAndUpdateStockId);
        } else {
            me.disableButton(me.saveAndUpdateStockId);
        }
    },
