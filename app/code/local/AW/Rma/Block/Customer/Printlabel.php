...
class AW_Rma_Block_Customer_Printlabel extends Mage_Core_Block_Template
{
    ...
    public function getFormPostUrl()
    {
        if ($this->getGuestMode()) {
            return $this->getUrl('awrma/guest_rma/printform', array('id' => $this->getRmaRequest()->getExternalLink(), '_secure' => Mage::app()->getStore(true)->isCurrentlySecure()));
        } else {
            return $this->getUrl('awrma/customer_rma/printform', array('id' => $this->getRmaRequest()->getId(), '_secure' => Mage::app()->getStore(true)->isCurrentlySecure()));
        }
    }
