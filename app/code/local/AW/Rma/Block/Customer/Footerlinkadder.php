        ...
        if (Mage::helper('awrma')->isEnabled()) {
            ...
            if ($parentBlock instanceof Mage_Page_Block_Template_Links) {
                if (is_null(Mage::getSingleton('customer/session')->getId())
                    ...
                    $parentBlock->addLink(
                        $this->__('Request RMA'), Mage::app()->getStore()->getUrl('awrma/guest_rma/index', array('_secure' => Mage::app()->getStore(true)->isCurrentlySecure())),
                        $this->__('Request RMA')
                    );
                } else {
                    $parentBlock->addLink(
                        $this->__('Request RMA'), Mage::app()->getStore()->getUrl('awrma/customer_rma/new', array('_secure' => Mage::app()->getStore(true)->isCurrentlySecure())),
                        $this->__('Request RMA')
                    );
                }
            }
