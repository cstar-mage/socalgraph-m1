    ...
    protected function saveAction()
    {
        ...
        if (!$this->hasErrors()) {
            ...
            if($this->getRequest()->getParam('update')){
                try {
                    $addedQty = $rmaRequest->updateItemsStock();
                    if ($addedQty > 0){
                        $this->_getSession()->addSuccess(Mage::helper('awrma')->__('Stock updated successfully: %d item(s) added to stock', $addedQty));
                    } else {
                        $this->_getSession()->addError(Mage::helper('awrma')->__('Cannot update stock: some error occurred'));
                    }
                } catch (Exception $ex) {
