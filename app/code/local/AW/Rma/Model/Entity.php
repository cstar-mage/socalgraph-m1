...
class AW_Rma_Model_Entity extends Mage_Core_Model_Abstract
{
    ...
    {
        if (is_array($this->getData('print_label'))){
            $printLabel = $this->getData('print_label');
            $streetAddress = $printLabel['streetaddress'];
            if(is_array($streetAddress)) {
                return ((string) (isset($streetAddress[0]) ? $streetAddress[0] : "") )
                . " " . ((string) (isset($streetAddress[1]) ? $streetAddress[1] : "") );
            }
        } else {
            return "";
        }
