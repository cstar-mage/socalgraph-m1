...
class AW_Rma_Model_Notify extends Mage_Core_Model_Abstract
{
    ...
    protected function _notifyByStatus($rmaRequest, $comment = null, $isFrontend = false)
    {
        ...
        # Notify admin
        $adminNotifiedStatus = $entityStatus;
        if ($this->getFlagNotifyNew() && Mage::helper('awrma/config')->getAllowAutoApproveRequest()) {
            $adminNotifiedStatus = Mage::getModel('awrma/entitystatus')->load(Mage::helper('awrma/status')->getPendingApprovalStatusId());
            $adminNotifiedStatus->storeTemplateFromRma($rmaRequest->getStoreId());
        }
        if ($this->_isEnabledForAdmin() && $adminNotifiedStatus->getToAdmin() && $isFrontend) {
            $rmaRequest->setData('notify_order_admin_link', Mage::helper('awrma')->getOrderUrl($rmaRequest->getOrderId()));
            $this->_notifyAdminOnStatus($rmaRequest, $adminNotifiedStatus, $comment);
        }
