...
class AW_Rma_Helper_Request extends Mage_Core_Helper_Abstract
{
                    ...
                    if ((Mage::helper('awrma/config')->getAllowPerOrderRMA() && count($_orderItems))
                        ...
                        if ($_addNewEntityFlag) {
                            ...
                            if (!($packageOpenedOption === false)) {
                                ...
                                $_data['status'] = self::getNewRequestStatus();
                                ...
                                $defaultPrintLabelData = self::getDefaultPrintLabelData($_order);
                                if ($defaultPrintLabelData) {
                                    $_data['print_label'] = $defaultPrintLabelData;
                                }
                                ...
                                if (self::getIsNeedNotifyWaitingForApproval()) {
    ...
    public static function getNewRequestStatus() {
        if (Mage::helper('awrma/config')->getAllowAutoApproveRequest()) {
            return Mage::helper('awrma/status')->getApprovedStatusId();
        } else {
            return Mage::helper('awrma/status')->getPendingApprovalStatusId();
        }
    }

    public static function getIsNeedNotifyWaitingForApproval() {
        return !(Mage::helper('awrma/config')->getAllowAutoApproveRequest());
    }
}