...
class AW_Rma_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public static function getOrderUrl($incrementId, $admin = true)
    {
        ...
        if ($_order->getData() != array()) {
            if ($admin) {
                ...
                return Mage::app()->getStore()->getUrl('sales/order/view', array('order_id' => $_order->getId(), '_secure' => Mage::app()->getStore(true)->isCurrentlySecure()));
            }
        } else {
            return null;
        }
    ...
    public function isUpdateStockEnabled($status)
    {
        $allowedStatuses = Mage::helper('awrma/status')->getAllowedStatusesUpdateStock();
    ...
    public function isNeedToShowConfirmPopup($rmaRequest)
    {
        if ($rmaRequest->getUpdateStockQty() > 0){
            return 1;
        } else {
            return 0;
        }
