    ...
    implements AW_Advancedreports_Model_Mysql4_Collection_Profit_Interface
{
    ...
    public function addOrderItems(
        ...
        $skuType = AW_Advancedreports_Model_System_Config_Source_Skutype::SKUTYPE_CHILD,
        $isAllStores = false
    )
    {
        ...
        if ($skuType == AW_Advancedreports_Model_System_Config_Source_Skutype::SKUTYPE_CHILD) {
    ...
    public function addProfitInfo($dateFrom, $dateTo, $isAllStores = false)
    {
        ...
        if ($this->getSkuType() == AW_Advancedreports_Model_System_Config_Source_Skutype::SKUTYPE_PARENT) {
                            ...
                            LEFT JOIN 
                                (
                                    ...
                                    FROM {$itemTable}
                                        INNER JOIN 
                                            {$orderTable} AS `order` 
                                            ON order.entity_id = {$itemTable}.order_id
                                    WHERE 
    ...
    protected function getSkuType()
    {
        return Mage::helper('advancedreports/setup')->getCustomConfig('advancedreports_bestsellers_options_skutype');
    }
    ...
    {
        $bundleChildCost = $this->getBundleChildCostExpression();
        return "
        (
            SUM(
                IFNULL(
    ...
    protected function getBundleChildCostExpression()
    {
        $bundleChildCost = 'NULL';
        if ($this->getSkuType() == AW_Advancedreports_Model_System_Config_Source_Skutype::SKUTYPE_CHILD) {
            // fills by zero bundle child cost for bundle products with fixed price (SKU of simple product mode)
            $bundleChildCost = "
                IF(
                    parent_item.product_type = 'bundle' AND IFNULL(item2.base_row_total, item.base_row_total) = 0, 
                    0, 
                    NULL
                )";
        }
        return $bundleChildCost;
    }
