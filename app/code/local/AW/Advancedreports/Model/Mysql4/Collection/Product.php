                            ...
                            LEFT JOIN 
                                (
                                    SELECT  parent_item_id, 
                                            ...
                                            SUM(orderItem.base_discount_amount) AS base_discount_amount 
                                    FROM 
                                        {$itemTable} AS orderItem
                                        INNER JOIN 
                                            {$orderTable} AS `order` 
                                            ON order.entity_id = orderItem.order_id
                                    WHERE 
                                        (
                                ...
                                AS `bundle_item` 
                                ON bundle_item.parent_item_id = item.item_id
                        WHERE
                            {$skuTypeCondition} 
                            AND 
                            (
        ...
        (
            (
                IFNULL(
                    COALESCE(
                        IF(
                            item.product_type = 'bundle' AND (bundle_item.base_price) <= 0, 
                            NULL,
                            IF(
                                item.product_type = 'bundle', 
                                COALESCE(
                                    bundle_item.base_cost, 
                                    0
                                ), 
                                bundle_item.base_cost
                            )
    ...
    public function getTotalDiscountExpression($currencyRate)
    {
        ...
        (
            COALESCE(
                IF(
                    item.product_type = 'bundle' AND bundle_item.base_price > 0, 
                    bundle_item.base_discount_amount, 
                    NULL
                ), 
