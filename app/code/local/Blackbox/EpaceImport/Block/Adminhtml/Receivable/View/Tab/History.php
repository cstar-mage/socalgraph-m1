<?php
/**
 * Receivable history tab
 *
 * @category   Mage
 * @package    Mage_Adminhtml
 * @author      Magento Core Team <core@magentocommerce.com>
 */
class Blackbox_EpaceImport_Block_Adminhtml_Receivable_View_Tab_History
    ...
    implements Mage_Adminhtml_Block_Widget_Tab_Interface
{
    ...
    {
        parent::_construct();
        $this->setTemplate('blackbox/epacei/receivable/view/tab/history.phtml');
    }
    ...
    /**
     * Compose and get receivable full history.
    ...
    public function getFullHistory()
    {
        ...
        $history = array();
        foreach ($receivable->getAllStatusHistory() as $receivableComment){
            $history[] = $this->_prepareHistoryItem(
                $receivableComment->getStatusLabel(),
                $receivableComment->getIsCustomerNotified(),
                $receivableComment->getCreatedAtDate(),
                $receivableComment->getComment()
            );
        }

        foreach ($receivable->getCreditmemosCollection() as $_memo){
            $history[] = $this->_prepareHistoryItem(
        ...
        foreach ($receivable->getShipmentsCollection() as $_shipment){
            $history[] = $this->_prepareHistoryItem(
        ...
        foreach ($receivable->getInvoiceCollection() as $_invoice){
            $history[] = $this->_prepareHistoryItem(
        ...
        foreach ($receivable->getTracksCollection() as $_track){
            $history[] = $this->_prepareHistoryItem(
    ...
    {
        return Mage::helper('epacei')->__('Receivable History');
    }
    ...
    {
        return $historyItem['notified'] == Blackbox_EpaceImport_Model_Receivable_Status_History::CUSTOMER_NOTIFICATION_NOT_APPLICABLE;
    }
