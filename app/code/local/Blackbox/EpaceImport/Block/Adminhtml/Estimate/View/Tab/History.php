 ...
 */
class Blackbox_EpaceImport_Block_Adminhtml_Estimate_View_Tab_History
    ...
    implements Mage_Adminhtml_Block_Widget_Tab_Interface
{
    ...
    {
        parent::_construct();
        $this->setTemplate('blackbox/epacei/estimate/view/tab/history.phtml');
    }
    ...
    /**
     * Compose and get estimate full history.
    ...
    public function getFullHistory()
    {
        ...
        $history = array();
        foreach ($estimate->getAllStatusHistory() as $estimateComment){
            $history[] = $this->_prepareHistoryItem(
                $estimateComment->getStatusLabel(),
                $estimateComment->getIsCustomerNotified(),
                $estimateComment->getCreatedAtDate(),
                $estimateComment->getComment()
            );
        }

        foreach ($estimate->getCreditmemosCollection() as $_memo){
            $history[] = $this->_prepareHistoryItem(
        ...
        foreach ($estimate->getShipmentsCollection() as $_shipment){
            $history[] = $this->_prepareHistoryItem(
        ...
        foreach ($estimate->getInvoiceCollection() as $_invoice){
            $history[] = $this->_prepareHistoryItem(
        ...
        foreach ($estimate->getTracksCollection() as $_track){
            $history[] = $this->_prepareHistoryItem(
    ...
    {
        return Mage::helper('epacei')->__('Estimate History');
    }
    ...
    {
        return $historyItem['notified'] == Blackbox_EpaceImport_Model_Estimate_Status_History::CUSTOMER_NOTIFICATION_NOT_APPLICABLE;
    }
