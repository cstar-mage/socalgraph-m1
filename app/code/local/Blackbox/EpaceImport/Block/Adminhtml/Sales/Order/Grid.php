...
class Blackbox_EpaceImport_Block_Adminhtml_Sales_Order_Grid extends Mage_Adminhtml_Block_Sales_Order_Grid
{
    protected function _prepareCollection()
    {
        ...
        $collection->getSelect()->join(['o' => $collection->getResource()->getTable('sales/order')], 'main_table.entity_id = o.entity_id', [
            'epace_job_id',
            'customer' => 'CONCAT(COALESCE(o.customer_firstname, \'\'), \' \', COALESCE(o.customer_lastname, \'\'))',
            'sales_person_id',
            'amount_to_invoice',
            'change_order_total'
        ])->joinLeft(['e' => $collection->getResource()->getTable('epacei/estimate')], 'o.estimate_id = e.entity_id', [
            'estimate_price' => 'base_grand_total',
            'estimate_currency_code'
        ])->columns(['delta' => 'o.amount_to_invoice - COALESCE(e.base_grand_total, 0)']);
