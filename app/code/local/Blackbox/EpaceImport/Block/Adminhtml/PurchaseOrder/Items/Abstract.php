 ...
 */
class Blackbox_EpaceImport_Block_Adminhtml_PurchaseOrder_Items_Abstract extends Mage_Adminhtml_Block_Template
{
    /**
     ...
     * @return Blackbox_EpaceImport_Block_Adminhtml_PurchaseOrder_Items_Abstract
     */
    ...
    {
        if ($item->getPurchaseOrderItem()) {
            $type = $item->getPurchaseOrderItem()->getProductType();
        } else {
    ...
    {
        $extraInfoBlock = $this->getChild('purchase_order_item_extra_info');
        if ($extraInfoBlock) {
    ...
    {
        if ($item->getPurchaseOrderItem()) {
            $block = $this->getColumnRenderer($column, $item->getPurchaseOrderItem()->getProductType());
        } else {
    ...
    public function getPurchaseOrder()
    {
        ...
        Mage::throwException(Mage::helper('sales')->__('Cannot get purchase order instance'));
    }
    ...
    {
        if ($code == 'tax_amount' && $this->getPurchaseOrder()->getRowTaxDisplayPrecision()) {
            return $this->displayRoundedPrices(
                $this->getPriceDataObject()->getData('base_'.$code),
                $this->getPriceDataObject()->getData($code),
                $this->getPurchaseOrder()->getRowTaxDisplayPrecision(),
                $strong,
                $separator
            );
        } else {
    ...
    public function displayRoundedPrices($basePrice, $price, $precision=2, $strong = false, $separator = '<br />')
    {
        if ($this->getPurchaseOrder()->isCurrencyDifferent()) {
            $res = '';
            $res.= $this->getPurchaseOrder()->formatBasePricePrecision($basePrice, $precision);
            $res.= $separator;
            $res.= $this->getPurchaseOrder()->formatPricePrecision($price, $precision, true);
        }
        else {
            $res = $this->getPurchaseOrder()->formatPricePrecision($price, $precision);
            if ($strong) {
    ...
    public function displayPriceInclTax(Varien_Object $item)
    {
        ...
        return $this->displayPrices(
            $this->getPurchaseOrder()->getStore()->roundPrice($basePriceTax),
            $this->getPurchaseOrder()->getStore()->roundPrice($priceTax)
        );
    }
    ...
    public function canCreateShipment()
    {
        foreach ($this->getInvoice()->getAllItems() as $item) {
            if ($item->getPurchaseOrderItem()->getQtyToShip()) {
                return true;
            }
        }
        return false;
    ...
    {
        return $this->getPurchaseOrder()->formatPrice($price);
    }
