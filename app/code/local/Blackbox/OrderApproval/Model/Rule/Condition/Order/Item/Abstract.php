...
/**
 * Abstract Rule order condition data model
 *
 ...
 */
abstract class Blackbox_OrderApproval_Model_Rule_Condition_Order_Item_Abstract extends Blackbox_OrderApproval_Model_Rule_Condition_Abstract
{
    /**
    ...
    public function getAttributeObject()
    {
        try {
            $obj = Mage::getSingleton('eav/config')
                ->getAttribute('order_item', $this->getAttribute());
        }
        catch (Exception $e) {
            $obj = new Varien_Object();
            $obj->setEntity(Mage::getResourceSingleton('sales/order_item'))
     ...
     *
     * @return Blackbox_OrderApproval_Model_Rule_Condition_Cms_Block_Abstract
     */
    public function loadAttributeOptions()
    {
        $connection = Mage::getSingleton('core/resource')->getConnection('core_read'); /* @var Zend_Db_Adapter_Abstract $connection */
        $fields = $connection->describeTable(Mage::getSingleton('core/resource')->getTableName('sales/order_item'));

        $attributes = array();

        foreach ($fields as $field) {
            $attributes[$field['COLUMN_NAME']] = $this->_convertIdToName($field['COLUMN_NAME']);
        }
     ...
     *
     * @param Mage_Cms_Model_Resource_Block_Collection $cmsBlockCollection
     * @return Blackbox_OrderApproval_Model_Rule_Condition_Cms_Block_Abstract
     */
    public function collectValidatedAttributes($cmsBlockCollection)
    {
        ...
        if ($this->getAttributeObject()->isScopeGlobal()) {
            ...
            $cmsBlockCollection->addAttributeToSelect($attribute, 'left');
        } else {
            $this->_entityAttributeValues = $cmsBlockCollection->getAllAttributeValues($attribute);
        }
    ...
    /**
     * Validate order item attrbute value for condition
     *
    ...
    public function validate(Varien_Object $object)
    {
        ...
        if (!($object instanceof Mage_Sales_Model_Order_Item) && !($object instanceof Mage_Sales_Model_Quote_Item)) {
            $object = Mage::getModel('sales/order_item')->load($object->getId());
        }

        if ($object instanceof Mage_Sales_Model_Quote_Item) {
            $attrCode = $this->_prepareQuoteItemAttribute($attrCode);
        }

        return $this->validateAttribute($object->getData($attrCode));
    }

    protected function _prepareQuoteItemAttribute($attrCode)
    {
        $attributes = array(
            'qty_ordered' => 'qty'
        );
